...
void QmitkMultiWidgetLayoutManager::SetLayoutDesign(LayoutDesign layoutDesign)
{
  ...
  {
    SetCustomAxialLeftCoronalSagittalMiddle3DRightLayout();
    break;
  }
...
void QmitkMultiWidgetLayoutManager::SetCustomAxialLeftCoronalSagittalMiddle3DRightLayout()
{
  MITK_INFO << "Set custom axial left, coronal/sagittal middle, 3D right layout" << std::endl;
  ...
  // 左侧：Axial
  auto axialWidget = m_MultiWidget->GetRenderWindowWidget(0, 0); // 假定(0,0)是Axial
  auto axialContainer = new QSplitter(Qt::Vertical, m_MultiWidget);
  axialContainer->addWidget(axialWidget.get());
  axialWidget->show();
  hBoxLayout->addWidget(axialContainer);

  // 中间：Coronal (上), Sagittal (下)
  auto middleSplitter = new QSplitter(Qt::Vertical, m_MultiWidget);
  auto coronalWidget = m_MultiWidget->GetRenderWindowWidget(1, 0);  // 假定(1,0)是Coronal
  auto sagittalWidget = m_MultiWidget->GetRenderWindowWidget(0, 1); // 假定(0,1)是Sagittal
  middleSplitter->addWidget(coronalWidget.get());
  coronalWidget->show();
  middleSplitter->addWidget(sagittalWidget.get());
  sagittalWidget->show();
  hBoxLayout->addWidget(middleSplitter);

  // 右侧：3D
  auto widget3D = m_MultiWidget->GetRenderWindowWidget(1, 1); // 假定(1,1)是3D
  auto rightContainer = new QSplitter(Qt::Vertical, m_MultiWidget);
  rightContainer->addWidget(widget3D.get());
  widget3D->show();
  ...
  for (auto &renderWindow : allRenderWindows)
  {
    renderWindow->UpdateLayoutDesignList(LayoutDesign::CUSTOM_AXIAL_LEFT_CORONAL_SAGITTAL_MIDDLE_3D_RIGHT);
  }
}
