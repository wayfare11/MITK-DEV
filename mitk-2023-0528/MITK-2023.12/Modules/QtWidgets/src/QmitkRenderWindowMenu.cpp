...
QmitkRenderWindowMenu::QmitkRenderWindowMenu(QWidget *parent, Qt::WindowFlags flags, mitk::BaseRenderer *baseRenderer)
...
void QmitkRenderWindowMenu::SetLayoutIndex(LayoutIndex layoutIndex)
{
  ...
  // 刷新按钮显隐
  if (m_ChangeBackgroundButton)
  {
    if (m_Layout == mitk::AnatomicalPlane::Original)
      m_ChangeBackgroundButton->setVisible(true);
    else
      m_ChangeBackgroundButton->setVisible(false);
  }

    if (m_DropdownButton)
  {
    if (m_Layout == mitk::AnatomicalPlane::Original)
      m_DropdownButton->setVisible(true);
    else
      m_DropdownButton->setVisible(false);
  }

  // 控制菜单宽度
  if (m_Layout == LayoutIndex::Original) // 这里换成你3D窗口的枚举值
  {
    setMinimumWidth(90);
    setMaximumWidth(90);
  }
...
void QmitkRenderWindowMenu::UpdateLayoutDesignList(LayoutDesign layoutDesign)
{
  ...
  switch (m_LayoutDesign)
  {
    ...
    case LayoutDesign::CUSTOM_AXIAL_LEFT_CORONAL_SAGITTAL_MIDDLE_3D_RIGHT:
    {
      m_myDesignAction->setEnabled(false);
      break;
    }
...
void QmitkRenderWindowMenu::CreateMenuWidget()
{
  ...
  QSize size2(15, 15);
  // 截图按钮
  m_ScreenshotButton = new QToolButton(this);
  m_ScreenshotButton->setMaximumSize(25, 25);
  m_ScreenshotButton->setObjectName("ScreenshotButton");
  m_ScreenshotButton->setIconSize(size1);
  m_ScreenshotButton->setIcon(QIcon(QPixmap(":/Qmitk/photo.png"))); // 使用合适的图标
  m_ScreenshotButton->setAutoRaise(true);
  layout->addWidget(m_ScreenshotButton);
  m_ScreenshotButton->setStyleSheet("background-color: black;");

  // 变色按钮
  m_ChangeBackgroundButton = new QToolButton(this);
  m_ChangeBackgroundButton->setMaximumSize(25, 25);
  m_ChangeBackgroundButton->setObjectName("ChangeBackgroundButton");
  m_ChangeBackgroundButton->setIconSize(size1);
  m_ChangeBackgroundButton->setIcon(QIcon(QPixmap(":/Qmitk/square.png"))); // 使用合适的图标
  m_ChangeBackgroundButton->setAutoRaise(true);
  layout->addWidget(m_ChangeBackgroundButton);
  m_ChangeBackgroundButton->setStyleSheet("background-color: black;");

  // 下拉按钮
  m_DropdownButton = new QToolButton(this);
  m_DropdownButton->setMaximumSize(15, 15);
  m_DropdownButton->setObjectName("DropdownButton");
  m_DropdownButton->setIconSize(size2);
  m_DropdownButton->setIcon(QIcon(QPixmap(":/Qmitk/dropdown.png"))); // 使用合适的图标
  m_DropdownButton->setAutoRaise(true);
  layout->addWidget(m_DropdownButton);
  m_DropdownButton->setStyleSheet("background-color: black;");


  if (m_CrosshairModeButton)
  {
    m_CrosshairModeButton->setVisible(false);
  }
  ...
  if (m_LayoutDesignButton)
  {
    m_LayoutDesignButton->setVisible(false);
  }
}
...
void QmitkRenderWindowMenu::CreateSettingsWidget()
{
  ...
  m_myDesignAction = new QAction("My design", m_LayoutActionsMenu);
  m_myDesignAction->setDisabled(false);
  ...
  m_LayoutActionsMenu->addAction(m_myDesignAction);
  ...
  connect(m_All2DTop3DBottomLayoutAction,
          &QAction::triggered,
          [this]() { this->OnSetLayout(LayoutDesign::ALL_2D_TOP_3D_BOTTOM); });
  connect(m_All2DLeft3DRightLayoutAction,
          &QAction::triggered,
          [this]() { this->OnSetLayout(LayoutDesign::ALL_2D_LEFT_3D_RIGHT); });
  connect(m_OneBigLayoutAction, &QAction::triggered, [this]() { this->OnSetLayout(LayoutDesign::ONE_BIG); });
  connect(m_Only2DHorizontalLayoutAction,
          &QAction::triggered,
          [this]() { this->OnSetLayout(LayoutDesign::ONLY_2D_HORIZONTAL); });
  connect(
    m_Only2DVerticalLayoutAction, &QAction::triggered, [this]() { this->OnSetLayout(LayoutDesign::ONLY_2D_VERTICAL); });
  connect(m_OneTop3DBottomLayoutAction,
          &QAction::triggered,
          [this]() { this->OnSetLayout(LayoutDesign::ONE_TOP_3D_BOTTOM); });
  connect(m_OneLeft3DRightLayoutAction,
          &QAction::triggered,
          [this]() { this->OnSetLayout(LayoutDesign::ONE_LEFT_3D_RIGHT); });
  connect(
    m_AllHorizontalLayoutAction, &QAction::triggered, [this]() { this->OnSetLayout(LayoutDesign::ALL_HORIZONTAL); });
  connect(m_AllVerticalLayoutAction, &QAction::triggered, [this]() { this->OnSetLayout(LayoutDesign::ALL_VERTICAL); });

  connect(m_myDesignAction,
          &QAction::triggered,
          [this]() { this->OnSetLayout(LayoutDesign::CUSTOM_AXIAL_LEFT_CORONAL_SAGITTAL_MIDDLE_3D_RIGHT); });
...
void QmitkRenderWindowMenu::OnTSNumChanged(int num)
{
  ...
  if (m_Renderer.IsNotNull())
  {
    ...
    if (m_Renderer->GetCurrentWorldPlaneGeometryNode()->GetProperty(resliceMethodEnumProperty, "reslice.thickslices") &&
...
void QmitkRenderWindowMenu::OnCrosshairMenuAboutToShow()
{
  ...
  // Show hide crosshairs
  {
    ...
    connect(
      showHideCrosshairVisibilityAction, &QAction::toggled, this, &QmitkRenderWindowMenu::OnCrosshairVisibilityChanged);
  }
  ...
  // Rotation mode
  {
    ...
    connect(
      rotationModeActionGroup, &QActionGroup::triggered, this, &QmitkRenderWindowMenu::OnCrosshairRotationModeSelected);
  }
