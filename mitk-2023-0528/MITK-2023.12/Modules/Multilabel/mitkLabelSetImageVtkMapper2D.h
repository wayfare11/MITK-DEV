...
namespace mitk
{
  ...
  public:
    ...
    public:
      ...
      /** Vector containing the pointer of the currently used layer images.
       * IMPORTANT: This member must not be used to access any data.
       * Its purpose is to allow checking if the order of the layers has changed
       * in order to adapt the pipeline accordingly.
       */
      std::vector<const Image *> m_LayerImageIDs;
      ...
      /** \brief Timestamp of last update of the active label outline. */
      ...
      unsigned int m_LastActiveLayer;
      mitk::Label::PixelType m_LastActiveLabelValue;
  ...
  protected:
    ...
    void GenerateImageSlice(mitk::BaseRenderer *renderer, const std::vector<unsigned int> &outdatedLayerIDs);
