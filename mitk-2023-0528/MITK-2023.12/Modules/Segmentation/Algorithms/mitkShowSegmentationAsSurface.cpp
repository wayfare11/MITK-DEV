...
namespace mitk
{
  ShowSegmentationAsSurface::ShowSegmentationAsSurface() : m_UIDGeneratorSurfaces("Surface_"), m_IsLabelSetImage(false)
  {
  }
    ...
    if (nullptr != labelSetImage)
    {
      ...
      for (decltype(numberOfLayers) layerIndex = 0; layerIndex < numberOfLayers; ++layerIndex)
      {
        ...
        for (auto labelIter = labelSet->IteratorConstBegin(); labelIter != labelSet->IteratorConstEnd(); ++labelIter)
        {
          ...
          {
            MITK_WARN << "Label \"" << labelIter->second->GetName()
                      << "\" didn't produce any smoothed surface data (try again without smoothing).";
            continue;
          }
